...
contract RewardsManager {
    ...
    mapping(uint256 => mapping(address => uint256))
        public badgerEmissionPerEpochPerVault; // Epoch data for each epoch badgerEmissionPerEpochPerVault[epochId][vaultAddress]

    mapping(uint256 => mapping(address => mapping(address => uint256)))
        public points; // Calculate points per each epoch points[epochId][vaultAddress][userAddress]
    mapping(uint256 => mapping(address => mapping(address => uint256)))
        public pointsWithdrawn; // Given point for epoch how many where withdrawn by user? pointsWithdrawn[epochId][vaultAddress][userAddress]
    ...
    function setEmission(
        ...
        IERC20(BADGER).safeTransferFrom(
            msg.sender,
            address(this),
            badgerAmount
        );
        ...
        badgerEmissionPerEpochPerVault[epochId][vault] +=
            endBalance -
            startBalance;
    }
    ...
    function sendExtraReward(
        ...
        additionalReward[currentEpoch][vault][extraReward] +=
            endBalance -
            startBalance;
    }
    ...
    function _getEmissionIndex(address vault, address user)
        internal
        returns (uint256)
    {
    ...
    function _getBalanceAtCurrentEpoch(address vault, address user)
        internal
        returns (uint256)
    {
        ...
        for (uint256 i = currentEpoch; i >= 0; i--) {
            ...
            if (
                epochs[i].blockstart >= lastBalanceChangeTime &&
                lastBalanceChangeTime < epochs[i].blockEnd
            ) {
        ...
        if (lastBalanceChangeEpoch != currentEpoch) {
            ...
            shares[currentEpoch][vault][user] = shares[lastBalanceChangeEpoch][
                vault
            ][user];
        }
    ...
    function _getTimeInEpochFromLastAccrue(address vault, address user)
        internal
        returns (uint256)
    {
        uint256 lastBalanceChangeTime = lastUserAccrue[vault][user];
        ...
        if (lastBalanceChangeTime > epochs[currentEpoch].blockstart) {
            require(
                block.timestamp < epochs[currentEpoch].blockEnd,
                "No epoch active"
            ); // I believe this require can be helpful if we're not in an active epoch, which hopefully we can avoid
