...
MIN_VAULT_B_REWARDS_TO_A = (
    1_000  ## The "true" "base" yield from B -> A (without adding self-emissions)
)
VAULT_B_REWARDS_TO_A = 100_000  ## 100 k ETH example

VAULT_B_MIN_SELF_EMISSION = 0
VAULT_B_SELF_EMISSIONS = (
    100_000_000  ## 100M ETH example - Exacerbates issue with B -> B Claim
)
VAULT_B_EMISSIONS_TO_OTHER = (
    1_000_000  ## TODO Inflates total supply but is not added to rewards
)
VAULT_B_HODLERS = (
    1_000_000  ## TODO Inflates total supply and dilutes all emissions (even from C)
)
...
def multi_claim_sim():
    ...
    for epoch in range(number_of_epochs):
        ...
        for user in range(number_of_users):
            ...
            user_total_rewards_fair = (
                emissions_b_b[epoch] * points_b[user][epoch] // divisor
            )
    ...
    total_rewards_points_b = (
        total_rewards_b * SECONDS_PER_EPOCH
    ...
    assert (
        unclaimable_points_rewards_b_epoch[0]
        == total_rewards_points_b - rewards_b[0] * SECONDS_PER_EPOCH
    )
    ## Last is equal to 0
    ...
    for epoch_index in range(number_of_epochs):
        ...
        print_if(
            (total_unclaimed_points + virtual_account_rewards * SECONDS_PER_EPOCH)
            // virtual_divisor
            * 100
        )
    ...
    if total_emissions_b_b > 0:
        ...
        print(
            "total_claimed_self_emissions_b + virtual_account_rewards / total_emissions_b_b * 100"
        )
        print(
            (total_claimed_self_emissions_b + virtual_account_rewards)
            / total_emissions_b_b
            * 100
        )
    ...
    return (
        total_b_obtainable
        - (total_claimed_b + total_claimed_self_emissions_b + virtual_account_rewards)
    ) / (total_b_obtainable)
