...
## Additional B Rewards (We call them D to separate) <- NOTE: May wanna remove noise for a time
...
def multi_claim_sim():
    ...
    for epoch in range(number_of_epochs):
        reward_c = (
            ## TODO: ADD VARIABLES -> VAULT_C_EMISSIONS_TO_B rename too
            (int(random() * VAULT_C_EMISSIONS_TO_B) + MIN_VAULT_B_REWARDS_TO_A)
            * 10**SHARES_DECIMALS
            if not DETERMINISTIC
            else (MIN_VAULT_B_REWARDS_TO_A) * 10**SHARES_DECIMALS
        )
        ...
        c_self_emissions_epoch = (
            ## TODO: VARS
            (int(random() * VAULT_C_SELF_EMISSIONS) + VAULT_B_MIN_SELF_EMISSION)
            * 10**SHARES_DECIMALS
            if not DETERMINISTIC
            else (VAULT_B_MIN_SELF_EMISSION) * 10**SHARES_DECIMALS
        )
