...
def test_invalid_epochId(initialized_contract, user, fake_vault, token):
    ...
    with brownie.reverts():
        initialized_contract.getUserNextEpochInfo(
            invalidEPOCH, fake_vault.address, user.address, 0
        )
    with brownie.reverts():
...
def test_bulk_claim_over_already_claimed(initialized_contract, user, fake_vault, token):
    ...
    startAccrueTimestamp = initialized_contract.lastUserAccrueTimestamp(
        nextEPOCH, fake_vault.address, user.address
    )
...
def test_bulk_claim_non_emitting_over_already_claimed(
...
def test_claim_non_emitting_revert(initialized_contract, user, fake_vault, token):
    ...
    assert (
        initialized_contract.pointsWithdrawn(
