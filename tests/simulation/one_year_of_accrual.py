...
def test_full_deposit_one_year_reference(initialized_contract, user, fake_vault, token):
    ...
    ## Claim rewards here
    tx = initialized_contract.claimRewardReferenceEmitting(
        EPOCH, fake_vault, token, user
    )
...
def test_full_deposit_claim_one_year_of_rewards(
    ...
    for x in range(1, 52):
        initialized_contract.addReward(
            x, fake_vault, token, REWARD_AMOUNT, {"from": user}
        )
        chain.sleep(initialized_contract.SECONDS_PER_EPOCH() + 1)
        chain.mine()
    ...
    ## Claim rewards here
    tx = initialized_contract.claimRewards(
        epochs_to_claim, vaults_to_claim, tokens_to_claim, users_to_claim
    )

    ## Verify you got the entire amount
    assert (
        token.balanceOf(user) == initial_reward_balance
    )  ## First reward is still inside for another epoch

    assert (
        tx.gas_used <= 6_000_000
    )  ## Run through simulation 5291637 gas from running the test
...
def test_full_deposit_claim_one_year_of_rewards_with_bulk_function_no_optimizations(
    ...
    tx = initialized_contract.claimBulkTokensOverMultipleEpochs(
        1, 52, fake_vault, [token], user, {"from": user}
    )
    ...
    assert (
        tx.gas_used <= 3_500_000
    )  ## 3276492 Run through simulation, you save 1.5 MLN gas via refunds

    ## Verify user balance is still properly tracked
    assert (
        initialized_contract.getBalanceAtEpoch(
            initialized_contract.currentEpoch(), fake_vault, user
        )[0]
        == INITIAL_DEPOSIT
    )


def test_full_deposit_claim_one_year_of_rewards_with_as_many_savings_as_i_could(
    initialized_contract, user, fake_vault, token
):
    INITIAL_DEPOSIT = 1e18
    REWARD_AMOUNT = 1e20
    ...
    for x in range(1, 52):
        ...
        chain.sleep(initialized_contract.SECONDS_PER_EPOCH() + 1)
        chain.mine()
        initialized_contract.accrueUser(x, fake_vault, initialized_contract)
        ## Accrue the contract to save gas on claims
...
def test_full_deposit_claim_one_year_of_rewards_with_as_many_savings_as_i_could_non_emitting(
    initialized_contract, user, fake_vault, token
):
    INITIAL_DEPOSIT = 1e18
    REWARD_AMOUNT = 1e20
