...
def test_claimBulkTokensOverMultipleEpochs_basic(
    ...
    assert (
        initialized_contract.pointsWithdrawn(EPOCH, fake_vault, user, token) == points
    )
    ...
    assert (
        initialized_contract.points(EPOCH, fake_vault, user) == points_before
    )  ## No increase


def test_claimBulkTokensOverMultipleEpochs_permissions(
    initialized_contract, user, fake_vault, token, second_user
):
    INITIAL_DEPOSIT = 1e18
    REWARD_AMOUNT = 1e20
  ...
  """
    with brownie.reverts("only ended epochs"):
        initialized_contract.claimBulkTokensOverMultipleEpochs(
            EPOCH, CURRENT_EPOCH, fake_vault, [token], user, {"from": user}
        )
  ...
  """
    ...
    with brownie.reverts("already claimed"):
        initialized_contract.claimBulkTokensOverMultipleEpochs(
            CURRENT_EPOCH, CURRENT_EPOCH, fake_vault, [token], user, {"from": user}
        )


def test_claimBulkTokensOverMultipleEpochs_cannot_use_old_balance(
    initialized_contract, user, fake_vault, token, second_user
):
    INITIAL_DEPOSIT = 1e18
    REWARD_AMOUNT = 1e20
    ...
    initialized_contract.claimBulkTokensOverMultipleEpochs(
        2, 3, fake_vault, [token], user, {"from": user}
    )
    initialized_contract.claimBulkTokensOverMultipleEpochs(
        2, 3, fake_vault, [token], second_user, {"from": second_user}
    )
  ...
  """
    ...
    assert (
        initialized_contract.getBalanceAtEpoch(2, fake_vault, user)[0]
        == INITIAL_DEPOSIT
    )
  ...
  """
    ...
    assert (
        initialized_contract.getBalanceAtEpoch(2, fake_vault, second_user)[0]
        == INITIAL_DEPOSIT
    )
    ## same
    assert (
...
def test_bulk_claim_no_points(
    ...
    initialized_contract.claimBulkTokensOverMultipleEpochs(
        1, 1, fake_vault, [token], second_user, {"from": user}
    )
...
def test_bulk_claim_revert(initialized_contract, user, fake_vault, token, second_user):
    ...
    with brownie.reverts():
        initialized_contract.claimBulkTokensOverMultipleEpochs(
            1, 0, fake_vault, [token], second_user, {"from": user}
        )
    ...
    with brownie.reverts():
        initialized_contract.claimBulkTokensOverMultipleEpochs(
            1, 1, fake_vault, [token, token], second_user, {"from": user}
        )
