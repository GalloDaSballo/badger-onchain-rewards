...
"""
  Checks that rewards are properly set via `addBulkRewards`
"""

def test_basic_add_reward_bulk(initialized_contract, user, fake_vault, token):
  REWARD_AMOUNT = 1e18
  ...
  initialized_contract.addBulkRewards(1, 1, fake_vault, token, [SECOND_REWARD_AMOUNT], {"from": user})

  assert initialized_contract.rewards(1, fake_vault, token) == REWARD_AMOUNT + SECOND_REWARD_AMOUNT

def test_basic_add_multiple_rewards_bulk(initialized_contract, user, fake_vault, token):
  REWARD_AMOUNT = 1e18
  token.approve(initialized_contract, MaxUint256, {"from": user})
  initialized_contract.addBulkRewards(1, 1, fake_vault, token, [REWARD_AMOUNT], {"from": user})
...
def test_must_have_balance_to_add(initialized_contract, user, fake_vault, wbtc):
  ...
  with brownie.reverts():
    initialized_contract.addBulkRewards(2, 2, wbtc, fake_vault, [1000], {"from": user})
...
def test_revert_if_wrong_epoch_math(initialized_contract, user, fake_vault, token):
  ...
  with brownie.reverts(""):
    initialized_contract.addBulkRewards(2, 1, fake_vault, token, [REWARD_AMOUNT, REWARD_AMOUNT], {"from": user})
  
  ## Revert due to underflow
  with brownie.reverts(""):
    initialized_contract.addBulkRewards(3, 1, fake_vault, token, [REWARD_AMOUNT, REWARD_AMOUNT, REWARD_AMOUNT], {"from": user})
