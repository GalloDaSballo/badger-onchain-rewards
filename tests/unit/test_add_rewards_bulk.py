...
def test_basic_add_multiple_rewards_bulk(initialized_contract, user, fake_vault, token):
    ...
    initialized_contract.addBulkRewards(
        1, 2, fake_vault, token, [REWARD_AMOUNT, REWARD_AMOUNT], {"from": user}
    )
    assert (
        initialized_contract.rewards(1, fake_vault, token)
...
def test_cant_add_rewards_in_the_past(initialized_contract, user, fake_vault, token):
    ...
    with brownie.reverts("Cannot add to past"):
        initialized_contract.addBulkRewards(
            0, 1, fake_vault, token, [REWARD_AMOUNT, REWARD_AMOUNT], {"from": user}
        )
...
def test_revert_if_wrong_epoch_math(initialized_contract, user, fake_vault, token):
    ...
    with brownie.reverts(""):
        initialized_contract.addBulkRewards(
            2, 1, fake_vault, token, [REWARD_AMOUNT, REWARD_AMOUNT], {"from": user}
        )
