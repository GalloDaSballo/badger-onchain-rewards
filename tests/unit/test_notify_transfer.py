...
def test_a_deposit_increases_user_balance_and_total_supply(
    ...
    initialized_contract.notifyTransfer(
        AddressZero, user, SECOND_AMOUNT, {"from": fake_vault}
    )

    assert (
        initialized_contract.shares(EPOCH, fake_vault, user)
        == INITIAL_DEPOSIT + SECOND_AMOUNT
    )
    assert (
        initialized_contract.totalSupply(EPOCH, fake_vault)
        == INITIAL_DEPOSIT + SECOND_AMOUNT
    )


def test_a_transfer_reduces_user_balance_increases_recipient_balance_total_supply_unchanged(
    initialized_contract, user, second_user, fake_vault, token
):
    EPOCH = 1
    INITIAL_DEPOSIT = 1e18
    SECOND_AMOUNT = 3e18
    ...
    initialized_contract.notifyTransfer(
        user, second_user, INITIAL_DEPOSIT, {"from": fake_vault}
    )

    assert initialized_contract.shares(EPOCH, fake_vault, user) == 0
    assert (
        initialized_contract.shares(EPOCH, fake_vault, second_user) == INITIAL_DEPOSIT
    )
    assert initialized_contract.totalSupply(EPOCH, fake_vault) == INITIAL_DEPOSIT

    ## Second User Mints
    initialized_contract.notifyTransfer(
        AddressZero, second_user, SECOND_AMOUNT, {"from": fake_vault}
    )

    assert (
        initialized_contract.shares(EPOCH, fake_vault, second_user)
        == INITIAL_DEPOSIT + SECOND_AMOUNT
    )
    assert (
        initialized_contract.totalSupply(EPOCH, fake_vault)
    ...
    ## First user mints
    initialized_contract.notifyTransfer(
        AddressZero, user, THIRD_AMOUNT, {"from": fake_vault}
    )
    ...
    assert (
        initialized_contract.totalSupply(EPOCH, fake_vault)
        == INITIAL_DEPOSIT + SECOND_AMOUNT + THIRD_AMOUNT
    )

    ## Transfer half to user two
    initialized_contract.notifyTransfer(
        user, second_user, THIRD_AMOUNT / 2, {"from": fake_vault}
    )
    ...
    assert (
        initialized_contract.shares(EPOCH, fake_vault, second_user)
        == INITIAL_DEPOSIT + SECOND_AMOUNT + THIRD_AMOUNT / 2
    )


def test_a_withdrawal_reduces_user_balance_and_total_supply(
    initialized_contract, user, fake_vault, token
):
    EPOCH = 1
    INITIAL_DEPOSIT = 1e18
    ...
    ## Burn second amount
    initialized_contract.notifyTransfer(
        user, AddressZero, SECOND_AMOUNT, {"from": fake_vault}
    )
...
def test_transfer_reverts(initialized_contract, user, fake_vault, second_user, token):
    ...
    with brownie.reverts():
        initialized_contract.notifyTransfer(
            user, second_user, SECOND_AMOUNT, {"from": fake_vault}
        )
...
def test_notify_transfer_reverts_if_send_to_self(
    initialized_contract, user, fake_vault, second_user
):
    INITIAL_DEPOSIT = 1e18
    ...
    with brownie.reverts():
        initialized_contract.notifyTransfer(
            user, user, INITIAL_DEPOSIT, {"from": fake_vault}
        )
