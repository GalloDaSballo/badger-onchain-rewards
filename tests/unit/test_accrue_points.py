...
def test_full_deposit_one_user(initialized_contract, user, fake_vault):
    ...
    difference = epochData[1] - initialized_contract.lastUserAccrueTimestamp(
        EPOCH, fake_vault, user
    )
    vault_difference = epochData[1] - initialized_contract.lastAccruedTimestamp(
        EPOCH, fake_vault
    )
    ...
    assert (
        initialized_contract.points(EPOCH, fake_vault, user) == 0
    )  ## Have yet to accrue
    ...
    assert approx(
        initialized_contract.points(EPOCH, fake_vault, user), EXPECTED_POINTS, 1
    )
...
def test_full_deposit_one_user_two_epochs(initialized_contract, user, fake_vault):
    ...
    assert approx(
        initialized_contract.points(EPOCH_TWO, fake_vault, user), EXPECTED_POINTS, 1
    )
    assert (
        initialized_contract.totalPoints(EPOCH_TWO, fake_vault) == EXPECTED_VAULT_POINTS
    )
