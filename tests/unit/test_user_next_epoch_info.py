...
def test_getUserNextEpochInfo_uses_timestamp_for_data(
  ...
  """
    ...
    result = initialized_contract.getUserNextEpochInfo(
        epoch, fake_vault, user, OBVIOUSLY_WRONG_DEPOSIT_VALUE
    )
    ...
    ## Because we've had an accrual, balance is read from storage
    second_result = initialized_contract.getUserNextEpochInfo(
        epoch + 1, fake_vault, user, OBVIOUSLY_WRONG_DEPOSIT_VALUE
    )
    ...
    ## Epoch points is greater than the ones we have in storage
    assert second_result[2] > initialized_contract.points(
        epoch + 1, fake_vault, user
    )  ## Cause it's zero
    ## And the pointsInStorage match
    assert second_result[3] == initialized_contract.points(
        epoch + 1, fake_vault, user
    )  ## It's zero
...
def test_epoch_basic_comprison_to_getBalanceAtEpoch(
    initialized_contract, user, fake_vault
):
    """
    ...
    from_storage_bal = initialized_contract.getBalanceAtEpoch(epoch, fake_vault, user)[
        0
    ]
    ...
    ## Storage read
    assert (
        initialized_contract.getUserNextEpochInfo(
            epoch, fake_vault, user, from_storage_bal
        )[0]
        == INITIAL_DEPOSIT
    )
    ...
    assert (
        initialized_contract.getUserNextEpochInfo(epoch, fake_vault, user, 0)[0]
        == INITIAL_DEPOSIT
    )
    ...
    assert (
        initialized_contract.getUserNextEpochInfo(
            new_epoch, fake_vault, user, from_storage_bal
        )[0]
        == initialized_contract.getBalanceAtEpoch(epoch, fake_vault, user)[0]
    )
    ...
    assert (
        initialized_contract.getUserNextEpochInfo(
            new_epoch, fake_vault, user, fake_value
        )[0]
        == fake_value
    )
    ...
    assert (
        initialized_contract.getUserNextEpochInfo(
            latest_epoch, fake_vault, user, random_value
        )[0]
        == initialized_contract.getBalanceAtEpoch(latest_epoch, fake_vault, user)[0]
    )

    ## Even 0
    assert (
        initialized_contract.getUserNextEpochInfo(latest_epoch, fake_vault, user, 0)[0]
        == initialized_contract.getBalanceAtEpoch(latest_epoch, fake_vault, user)[0]
    )
