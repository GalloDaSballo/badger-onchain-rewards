...
def test_equivalence_points_reference_and_getUserNextEpochInfo(
    ...
    expected = initialized_contract.getUserNextEpochInfo(
        EPOCH, fake_vault, user, balance
    )
    expected_points = expected[2]

    expected_vault = initialized_contract.getVaultNextEpochInfo(
        EPOCH, fake_vault, balance
    )
